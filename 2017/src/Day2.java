    ...
    public static void main(String[] args){
        ...
        for(String line : lines){
            ...
            for (String item : lineParts){
                inputLine.add(Integer.parseInt(item));
            }

            checkSum = checkSum + minMaxDiff(inputLine);
        }

        System.out.println("CheckSum = " + checkSum);
    }

    public static Integer minMaxDiff(ArrayList<Integer> lineInput){

        int min = lineInput.get(0);
        int max = 0;

        for(int i=0; i<lineInput.size(); i++){
            min = (lineInput.get(i) < min) ? lineInput.get(i) : min;
            max = (lineInput.get(i) > max) ? lineInput.get(i) : max;
        }
